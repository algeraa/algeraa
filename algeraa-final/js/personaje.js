...
var BombasList;
export var pocionPList, pocionMList, pocionGList, flechasInventario, timer;
...
export var curaP, curaM, curaG, flechaI, bomb, sword, bow, mon, h, contadorH;
var pedestal_escudo;
var lg_escudo;
export var aura;
var text;
...
export function cargarSprites()
{
	...
	this.load.image('mensaje','assets/sprites/mensajeH.png');
	this.load.image('ayuda','assets/sprites/ayudaTeclas.png');
}
...
export function createP()
{
	...
	help=this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.M);
	...
	h=this.add.sprite(player.x, player.y, 'mensaje');
	h.setAlpha(0);
	h.setDepth(11);
    ...
    delay: 100,
    callback: ()=>{
        if(player.alpha==1){player.alpha = 0; console.log("parpadeo");}
        else{player.alpha = 1;}
    },
    loop: true,
    paused:true
...
function pararParpadeo()
{
	...
	timer.paused = true;
	if(player.alpha == 0){player.alpha = 1;}
}
export function perderVida(n, s)
{
	...
	if(player.couldownDamage <= 0)
	{
		...
		timer.paused = false;	
		var timer3 = this.time.delayedCall(1000, pararParpadeo, [], this);
	}
...
export function ayudar(){
	...
	if(help.isDown && invent==false && taim<=0 && helpu==false){
		...
		if(games.escenaActual == 1){
			...
			aiuda.x=cueva.cameras.scrollX+400;
			aiuda.y=cueva.cameras.scrollY+300;
			aiuda.setDepth(5);
			
		}
		else if(games.escenaActual==3){

			aiuda.x=escenaCastillo.cameras.scrollX+400;
			aiuda.y=escenaCastillo.cameras.scrollY+300;
			aiuda.setDepth(5);
