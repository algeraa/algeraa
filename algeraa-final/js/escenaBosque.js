...
var map;
var fl;
var ArbolesC, caveBlock, castleBlock, castleNoBlock;
...
export default class Bosque extends Phaser.Scene {
	...
	preload() {
		...
		this.load.image('bloqueoCu', 'assets/sprites/bloqueoCueva.png');
		this.load.image('bloqueoCa', 'assets/sprites/puertaCerrada.png');
		this.load.image('NobloqueoCa', 'assets/sprites/puertaAbierta.png');
...
create()
{
	...
	if(!personaje.llaveCueva){
		...
		caveBlock = this.physics.add.sprite(40,885, 'bloqueoCu');
		caveBlock.setImmovable(true);
	}
	else
	{
		pedestalSprite = pedestalList.create(800, 1600,'pedestal');
	...
	{
		castleBlock = this.physics.add.sprite(1205,115, 'bloqueoCa').setDepth(10);
		castleBlock.setImmovable(true);
	}
	else if(personaje.player.tienellave)
	{
		castleNoBlock = this.add.sprite(1205,115, 'NobloqueoCa').setDepth(10);
	}
	...
	this.physics.add.collider(personaje.player, caveBlock);
	this.physics.add.collider(personaje.player, castleBlock);
...
function abrirCueva()
{
		...
		caveBlock.setAlpha(0);
		caveBlock.body.enable = false;
}
...
function derribarArbol(s,n)
{
	...
	{
		if(n == arbolesList.getChildren()[1])
		{	
			n.body.enable = false;
			n.angle = 90;
			n.x=n.x+22;
